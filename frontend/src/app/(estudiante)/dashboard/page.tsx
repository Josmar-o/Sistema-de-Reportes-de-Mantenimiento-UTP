...
export default function EstudianteDashboard() {
  ...
  const fetchMisReportes = async () => {
    try {
      ...
      const stats = {
        total: data.length,
        pendientes: data.filter((r: Reporte) => r.estado === 'pendiente').length,
        enProceso: data.filter((r: Reporte) => r.estado === 'en_proceso').length,
        resueltos: data.filter((r: Reporte) => r.estado === 'resuelto').length,
      };
      setStats(stats);
    } catch (error: any) {
